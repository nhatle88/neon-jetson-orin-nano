...
def is_live_frame(current_frame, previous_frame, diff_threshold=2.0):
    """
    Compares the current frame with the previous frame. Returns a tuple (is_live, mean_diff).
    A low mean difference suggests that the frame is nearly identical to the previous one,
    which can be an indication of a static image (spoof).
    """
...
class RTSPStream:
    """Manages one RTSP stream and processes frames for face recognition and liveness check."""
    def __init__(self, name, rtsp_url, face_recognizer):
        ...
        self.running = True
        self.prev_frame = None  # For comparing consecutive frames (anti-spoofing)
    ...
    def process_stream(self):
        ...
        while self.running:
            ...
            # Anti-spoofing: Compare with previous frame if available
            if self.prev_frame is not None:
                live, mean_diff = is_live_frame(frame, self.prev_frame, diff_threshold=2.0)
                # If the frame difference is too low, we suspect a spoof (static image)
                if not live:
                    cv2.putText(frame, "Spoof Detected", (10, 30), cv2.FONT_HERSHEY_SIMPLEX,
                                1, (0, 0, 255), 2)
                    # Update the shared frame and continue to next frame without recognition
                    with frame_lock:
                        latest_frames[self.name] = frame.copy()
                    self.prev_frame = frame.copy()
                    continue

            # Process the frame for face recognition if it appears live
